...
A plan's directory holds its `README.md` – the plan itself, copied from [`TEMPLATE.md`](./TEMPLATE.md) – plus any supporting artifacts. The slug is the plan's permanent identity – the directory's are never renamed.

## How it works

1. A plan is opened as a draft pull request, on a `plan/<slug>` branch. The main document lives at `plans/<slug>/README.md`.
