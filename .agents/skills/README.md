...
- **[`/implement-plan`](./implement-plan/)**: `PLANNED` → `IN PROGRESS` — Marks the plan as underway once implementation has begun. The pull request and discussion thread stay open, and the plan stays mutable.

- **[`/complete-plan`](./complete-plan/)**: `IN PROGRESS` → `DONE` — Confirms every task has shipped (by following each linked tracker), closes the discussion thread, squash-merges the pull request to `main`, and appends the plan to `plans/INDEX.md`.

- **[`/abandon-plan`](./abandon-plan/)**: `PLANNED`/`IN PROGRESS` → `ABANDONED` — Drops a plan before completion, recording the reason, closes the discussion thread, and merges it to `main` as a permanent record.

A typical journey runs `/draft-plan` → the user writes the plan → `/finalize-plan` → review → `/implement-plan` → implementation across the code repos → `/complete-plan`. If the plan is dropped, `/abandon-plan` retires it instead.
