...
- **[`/implement-plan`](./implement-plan/)**: `PLANNED` → `IN PROGRESS` — Marks the plan as underway once review has concluded and implementation has begun. Closes the discussion thread. The pull request stays open and the plan stays mutable.
...
- **[`/abandon-plan`](./abandon-plan/)**: `PLANNED`/`IN PROGRESS` → `ABANDONED` — Drops a plan before completion, recording the reason, and merges it to `main` as a permanent record.
