...
the pull request into `main`, closes the discussion thread, and appends the
plan to the plan index.
...
the `#in-progress` pull requests and asks which plan to complete. It also
...
Run it on a `plan/<slug>` branch, or from `main` to pick from the
...
not merged into `main` until it is decided, so there is no intermediate
