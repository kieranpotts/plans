...
-   **The metadata header is filled in.**
    ...
    `Authors`, `Created`, `Last updated`, `Plan PR`, and `Target repositories` are set; the `Discussion thread` field links the thread (which stays open until the plan is done or abandoned); `Status` is `DRAFT` (this skill advances it to `PLANNED`).
